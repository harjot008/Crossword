...
class CrosswordCreator():
    ...
    def ac3(self, arcs=None):
        ...
        if arcs is None:
            queue = deque([(x, y)
                          for x in self.crossword.variables for y in self.crossword.neighbors(x)])
        else:
            queue = deque(arcs)
    ...
    def consistent(self, assignment):
        ...
        # Ensure all words are the correct length
        for var, word in assignment.items():
            if len(word) != var.length:
                return False

        # Ensure neighboring variables overlap consistently
